...
//	}
	...
	public static String encodeHttpFilename(String url){
		String path = url.substring(0,url.lastIndexOf("/")+1);
		String encodeFileName = encode(url.substring(url.lastIndexOf("/")+1,url.length()));
		return path+encodeFileName;
	}
	...
	public static String getFileName(String path)
	{
		try
		{
			return path.contains("/") ? path.substring(path.lastIndexOf("/")+1, path.length()) : "";
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public static String capitalizeString(String name)
	{
		if(!isEmpty(name)) {
			...
			for(String word : words)
			{
				...
				{
					if(isEmpty(capitalisedString))
						capitalisedString += word.substring(0, 1).toUpperCase() + word.substring(1).toLowerCase();
					else
						capitalisedString += " "+ word.substring(0, 1).toUpperCase() + word.substring(1).toLowerCase();
				}
			}
